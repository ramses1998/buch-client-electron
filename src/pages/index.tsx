...
const Home = () => {
  ...
  const resolveMostPopularBooks = (buecher: Buch[]): Buch[] => {
    return buecher.filter((b) => b.rating >= MINIMUM_RATING_FOR_POPULAR_BOOK);
  };
    ...
    <PageWrapperComponent>
      <Stack direction="column" spacing={"var(--gap-7)"}>
        <OverviewContainerWithItemList>
          {buecher.slice(0, MAX_BOOKS_COUNT_FOR_OVERVIEW).map((buch, index) => (
        ...
        >
          ...
          <ListContainer>
            {sortBooksByRating(resolveMostPopularBooks(buecher)).map(
              (buch, index) => (
                <BookCardComponent
                  key={index}
                  buch={buch}
